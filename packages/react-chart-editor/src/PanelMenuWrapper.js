...
import SidebarGroup from 'react-chart-editor/lib/components/sidebar/SidebarGroup';
import {bem} from 'react-chart-editor/lib/lib';
import sortMenu from 'react-chart-editor/lib/lib/sortMenu';
...
class PanelMenuWrapper extends Component {
  ...
  render() {
    ...
    if(menuOpts.find(e => e.name === this.state.group) == undefined){
      const firstSidebarGroup = menuOpts.filter((o) => o.panels)[0];
      this.state={
        group: firstSidebarGroup.name,
        panel: firstSidebarGroup.panels[0],
      };
