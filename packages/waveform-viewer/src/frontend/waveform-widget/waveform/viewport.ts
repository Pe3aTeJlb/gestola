...
import { NetlistData, arrayMove, NetlistId, ActionType } from "./helper";
...
export class Viewport {
  ...
  constructor(
    ...
    this.widget.events.subscribe(ActionType.Zoom, this.handleZoom);
    this.widget.events.subscribe(ActionType.ReorderSignals, this.handleReorderSignals);
    this.widget.events.subscribe(ActionType.AddVariable, this.handleAddVariable);
    this.widget.events.subscribe(ActionType.RemoveVariable, this.handleRemoveVariable);
    this.widget.events.subscribe(ActionType.RedrawVariable, this.handleRedrawSignal);
    this.widget.events.subscribe(ActionType.Resize, this.updateViewportWidth);
...
/*
  ...
  handleAddVariable(netlistIdList: NetlistId[], updateFlag: boolean) {
    netlistIdList.forEach((netlistId) => {
      if (!this.widget.dataManager.netlistData[netlistId]) {return;}
  ...
  handleScrollAreaMouseDown(event: MouseEvent) {
    ...
    if (event.button === 1) {
      ...
      this.widget.viewerState.mouseupEventType    = 'markerSet';
  ...
  handleScrollAreaClick(event: any, eventButton: number) {
    ...
    if (netlistId !== undefined && netlistId !== null) {
      ...
      const nearestTransition = this.widget.dataManager.getNearestTransition(netlistId, time);
    ...
    this.widget.events.dispatch(ActionType.MarkerSet, snapToTime, button);
  }
  ...
  highlightZoom() {
    ...
    this.widget.events.dispatch(ActionType.Zoom, amount, time, this.halfViewerWidth);
  }
  ...
  drawHighlightZoom(event: MouseEvent) {
    ...
    if (width > 5) {this.widget.viewerState.mouseupEventType = 'highlightZoom';}
    ...
    if (!this.highlightDebounce) {
      this.highlightDebounce = setTimeout(() => {
        this.widget.viewerState.mouseupEventType  = 'highlightZoom';
      }, 300);
    }
  }
  ...
  updateMarker() {
    if (this.markerElement && this.widget.viewerState.markerTime !== null) {
      this.markerElement.style.left = this.getViewportLeft(this.widget.viewerState.markerTime, 10) + 'px';
    }
    if (this.altMarkerElement && this.widget.viewerState.altMarkerTime !== null) {
      this.altMarkerElement.style.left = this.getViewportLeft(this.widget.viewerState.altMarkerTime, 10) + 'px';
    }
  }

  renderAllWaveforms(skipRendered: boolean) {
    const netlistData = this.widget.dataManager.netlistData;
    ...
    this.widget.viewerState.displayedSignals.forEach((netlistId, i) => {
  ...
  renderWaveform(netlistData: NetlistData) {
    ...
    const startIndex   = Math.max(this.widget.dataManager.binarySearch(valueChanges, this.timeScrollLeft - (2 * this.pixelTime)), 1);
    const endIndex     = this.widget.dataManager.binarySearch(valueChanges, this.timeScrollRight);
  ...
  handleReorderSignals(oldIndex: number, newIndex: number) {
    ...
    const netlistElement = this.widget.dataManager.netlistData[this.widget.viewerState.displayedSignals[newIndex]];
  ...
  handleRemoveVariable(netlistId: NetlistId) {
    ...
    this.waveformArea.replaceChildren(...children);
    const netlistElement = this.widget.dataManager.netlistData[netlistId];
    if (!netlistElement) {return;}
  ...
  handleMarkerSet(time: number, markerType: number) {
    ...
    if (markerType === 0) {
      this.widget.viewerState.markerTime = time;
      this.moveViewToTime(time);
    } else {
      this.widget.viewerState.altMarkerTime = time;
    }
  ...
  updateScrollContainer() {
    ...
    this.annotateScrollContainer(this.markerAnnotation , this.widget.viewerState.markerTime);
    this.annotateScrollContainer(this.markerAnnotation , this.widget.viewerState.altMarkerTime);
  }

  annotateScrollContainer(color: any, time: any) {
  ...
  handleSignalSelect(netlistId: NetlistId | null) {
    ...
    let element = document.getElementById('waveform-' + this.widget.viewerState.selectedSignal);
    if (element && this.widget.viewerState.selectedSignal !== null) {element.classList.remove('is-selected');}
  ...
  handleRedrawSignal(netlistId: NetlistId) {
    if (this.widget.viewerState.markerTime !== null) {
      this.widget.labelsPanel.valueAtMarker[netlistId] = this.widget.dataManager.getValueAtTime(netlistId, this.widget.viewerState.markerTime);
    }
  ...
  public updateViewportWidth() {
    ...
    this.widget.dataManager.netlistData.forEach((netlistItem) => {
